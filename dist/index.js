...
import { z } from "zod";
import { LISTINGS, HARVESTED_AT } from "./data.js";
const LISTINGS_BY_KEY = new Map();
for (const raw of LISTINGS) {
    const l = { ...raw };
    LISTINGS_BY_KEY.set(l.slug.toLowerCase(), l);
    LISTINGS_BY_KEY.set(l.title.toLowerCase(), l);
}
function findListing(idOrTitle) {
...
const server = new McpServer({ name: "earn-dataset-mcp", version: "1.0.0" }, { instructions: "Search and inspect a static snapshot of Superteam Earn (superteam.fun) bounty listings — 28 open listings with full description text, harvested 2026-09-04. " +
...
server.tool("list_listings", "List all 28 Superteam Earn listings in the dataset (snapshot 2026-09-04): title, reward, token, deadline, URL.", {}, async () => ({
    content: [{
            type: "text",
            text: JSON.stringify({
...
server.tool("get_listing", "Get one listing by slug, exact title, or unique title fragment — returns the FULL description text (sponsor brief, submission requirements, judging criteria).", { idOrTitle: z.string().describe("Listing slug, exact title, or unique title fragment, e.g. 'breakpoint-2026' or 'Create Content for Breakpoint 2026'") }, async ({ idOrTitle }) => {
    const l = findListing(idOrTitle);
    if (!l) {
        const matches = LISTINGS.filter((x) => x.title.toLowerCase().includes(idOrTitle.trim().toLowerCase())).map((x) => x.title);
...
server.tool("search_listings", "Keyword search across all listing titles and full description texts. All terms must match; results ranked by term frequency (title hits weigh more).", { query: z.string().describe("Keywords, e.g. 'content Solana' or 'QA tester'") }, async ({ query }) => {
...
server.tool("stats", "Dataset shape: listing count, harvest date, total reward pool, reward range, token mix, soonest deadlines.", {}, async () => {
    const ls = LISTINGS;
    const rewards = ls.map((l) => l.reward).filter((r) => typeof r === "number" && r > 0);
    const tokens = {};
    for (const l of ls)
        tokens[l.token] = (tokens[l.token] || 0) + 1;
